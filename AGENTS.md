...
- **Icons live in one file**: `components/icons.ts` exports an `icons` record of SVG path data (`name -> d` attribute), rendered by `components/Icon.tsx` as `<Icon name="copy" size={16} />`. Add an icon by adding an entry there — don't inline SVG in components, add a sprite file, or reach for an icon package. `Icon`'s `name` prop is typed as `keyof typeof icons`, so a typo is a `tsc` error rather than a silently blank icon. Paths are inlined into the bundle (no extra request, no flash-of-missing-icon); revisit that if the set ever grows past ~30 icons. Source icons from [Phosphor](https://phosphoricons.com) regular weight — canonical paths are in `phosphor-icons/core` under `assets/regular/<name>.svg`; paste only the `d` attribute, since all icons share Phosphor's `0 0 256 256` viewBox and inherit color via `fill="currentColor"` on the wrapper. `lucide-react` remains a dependency but only for `ArrowUpRight` in `components/Ledger.tsx`.
- **Writing archive**: `/writing` is a two-tab ledger — `All Writing / Archive` — toggled by client state in `app/writing/page.tsx` (inactive tab is `--ink-3`). The split comes from `isArchived()`/`ARCHIVE_BEFORE_YEAR` in `data/articles.ts` (anything dated before 2024). Only the Archive tab shows the year in the `LedgerRow` gutter; the home and All Writing lists pass no `gutter` (a `LedgerRow` without one drops the 7rem gutter column entirely). The `/ai/writing` twin mirrors the split as `## All Articles` + `## Archive` sections. `getRecentArticles()` (used by both home pages) filters archived posts out, so archived writing is reachable only from the Archive tab.
